...
module LGS = struct
  ...
  let f_loss ~data (theta : P.M.t) =
    ...
    in
    Maths.(loss_total / f (Float.of_int tmax))
  ...
  let init =
    ...
    let _c_params =
      Convenience.gaussian_tensor_2d_normed
        ...
        ~sigma:1.
    in
    let _std_o_params = Tensor.(zeros ~device:base.device ~kind:base.kind [ 1 ]) in
    let _std_space_params = Tensor.(zeros ~device:base.device ~kind:base.kind [ m ]) in
    let _std_time_params = Tensor.(zeros ~device:base.device ~kind:base.kind [ tmax ]) in
    PP.
      { _S_params
      ; _L_params
      ; _B_params
...
let test_LQR () =
  ...
  let data = o_list in
  let f = LGS.f_loss ~data in
  let _, _, e = check_grad ~f LGS.init in
  Alcotest.(check @@ rel_tol) "LQR test" 0.0 e

let () =
  Alcotest.run
    "LQR tests"
    [ ( "lqr_vae_elbo_simplified"
