...
type unary = string * input_constr list * (int list -> t -> t)
type binary = string * input_constr list * (int list -> t -> t -> t)
...
let unary_tests =
  let test_list : unary list =
    ...
    ; "erf", [], any_shape erf
    ; "inv", [ `specified_unary [ 4; 4 ] ], any_shape inv
  ...
  , fun () ->
      ...
      |> List.iter ~f:(fun _ ->
        ...
        Alcotest.(check @@ rel_tol)
          name
          0.0
          (check_grad2 (fun (x, y) -> f x y) (const x) (const y))) )

let matmul_with_einsum a b = einsum [ a, "ij"; b, "jk" ] "ik"
let batch_matmul_with_einsum_33 a b = einsum [ a, "mij"; b, "mjk" ] "mik"
let batch_matmul_with_einsum_32 a b = einsum [ a, "mij"; b, "mj" ] "mi"
...
let linsolve_tri ~left ~upper a b =
  let a =
    match upper with
    | false -> tril a ~_diagonal:0
    | true -> btr (tril a ~_diagonal:0)
  in
  linsolve_triangular a b ~left ~upper

let concat_list a b = concat [ a; b ] ~dim:0
...
let binary_tests =
  let test_list : binary list =
    ...
    ; "minus", [], any_shape ( - )
    ; "mult", [], any_shape ( * )
    ; "div", [ `positive ], any_shape ( / )
    ; "matmul", [ `matmul ], any_shape ( *@ )
    ...
    ; "kron", [ `specified_binary ([ 3; 4 ], [ 5; 7 ]) ], any_shape kron
