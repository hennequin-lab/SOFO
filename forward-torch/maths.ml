...
let dropout1d ?(p = 0.5) ?(is_training = true) (x, dx) =
  let y = Tensor.dropout x ~p ~is_training in
  let dy =
    with_tangent dx ~f:(fun dx ->
      if is_training
      then (
        (* list of single tensor which indicates position of non zero elements. *)
        let non_zero_list = Tensor.nonzero_numpy y |> List.hd_exn |> Tensor.to_list in
        let map_y = Tensor.zeros_like y in
        List.iter non_zero_list ~f:(fun non_zero_pos ->
          let x = Tensor.to_int0_exn non_zero_pos in
          Tensor.set_float1 map_y x 1.);
        let dy = Tensor.(map_y * dx) in
        dy)
      else dx)
  in
  (y, dy) |> assert_right_shape "dropout1d"

let dropout2d ?(p = 0.5) ?(is_training = true) (x, dx) =
  let y = Tensor.dropout x ~p ~is_training in
  let dy =
    with_tangent dx ~f:(fun dx ->
      if is_training
      then (
        (* list of tensors where each tensor is an array corresponding to numpy.nonzero. *)
        let non_zero_list = Tensor.nonzero_numpy y in
        let non_zero_list_ext =
          List.map non_zero_list ~f:(fun i ->
            Tensor.reshape i ~shape:(Tensor.shape i @ [ 1 ]))
        in
        (* list of tensors where each tensor is the (x, y) index of a nonzero element *)
        let non_zero_list_corrected =
          Tensor.concat non_zero_list_ext ~dim:1 |> Tensor.to_list
        in
        let map_y = Tensor.zeros_like y in
        List.iter non_zero_list_corrected ~f:(fun non_zero_pos ->
          let x, y = Tensor.get_int1 non_zero_pos 0, Tensor.get_int1 non_zero_pos 1 in
          Tensor.set_float2 map_y x y 1.);
        let dy = Tensor.(map_y * dx) in
        dy)
      else dx)
  in
  (y, dy) |> assert_right_shape "dropout2d"

let dropout3d ?(p = 0.5) ?(is_training = true) (x, dx) =
  let y = Tensor.dropout x ~p ~is_training in
  let dy =
    with_tangent dx ~f:(fun dx ->
      if is_training
      then (
        let[@warning "-8"] (_ :: batch_size :: _) = Tensor.shape dx in
        let batch_list =
          List.init batch_size ~f:(fun i ->
            let y_i =
              Tensor.slice y ~dim:0 ~start:(Some i) ~end_:(Some (Int.succ i)) ~step:1
              |> Tensor.squeeze
            in
            (* list of tensors where each tensor is an array corresponding to numpy.nonzero. *)
            let non_zero_list = Tensor.nonzero_numpy y_i in
            let non_zero_list_ext =
              List.map non_zero_list ~f:(fun i ->
                Tensor.reshape i ~shape:(Tensor.shape i @ [ 1 ]))
            in
            ...
            let non_zero_list_corrected =
              Tensor.concat non_zero_list_ext ~dim:1 |> Tensor.to_list
            in
            let map_y_i = Tensor.zeros_like y_i in
            List.iter non_zero_list_corrected ~f:(fun non_zero_pos ->
              let x, y = Tensor.get_int1 non_zero_pos 0, Tensor.get_int1 non_zero_pos 1 in
              Tensor.set_float2 map_y_i x y 1.);
            Tensor.reshape map_y_i ~shape:(1 :: Tensor.shape map_y_i))
        in
        let map_batch_y = Tensor.concat batch_list ~dim:0 in
        Tensor.(map_batch_y * dx))
      else dx)
  in
  (y, dy) |> assert_right_shape "dropout3d"

let dropout4d ?(p = 0.5) ?(is_training = true) (x, dx) =
  let y = Tensor.dropout x ~p ~is_training in
  let dy =
    with_tangent dx ~f:(fun dx ->
      if is_training
      then (
        (* list of tensors where each tensor is the index of nonzero elements. *)
        let[@warning "-8"] (_ :: batch_size :: nc :: _) = Tensor.shape dx in
        let batch_list =
          List.init batch_size ~f:(fun i ->
            let y_i =
              Tensor.slice y ~dim:0 ~start:(Some i) ~end_:(Some (Int.succ i)) ~step:1
              ...
              List.init nc ~f:(fun j ->
                let y_ij =
                  Tensor.slice
                    ...
                    ~dim:0
                    ~start:(Some j)
                    ~end_:(Some (Int.succ j))
                    ~step:1
                  |> Tensor.squeeze
                in
                (* list of tensors where each tensor is an array corresponding to numpy.nonzero. *)
                let non_zero_list = Tensor.nonzero_numpy y_ij in
                let non_zero_list_ext =
                  List.map non_zero_list ~f:(fun i ->
                    Tensor.reshape i ~shape:(Tensor.shape i @ [ 1 ]))
                in
                ...
                let non_zero_list_corrected =
                  Tensor.concat non_zero_list_ext ~dim:1 |> Tensor.to_list
                in
                let map_y_ij = Tensor.zeros_like y_ij in
                List.iter non_zero_list_corrected ~f:(fun non_zero_pos ->
                  let x, y =
                    Tensor.get_int1 non_zero_pos 0, Tensor.get_int1 non_zero_pos 1
                  in
                  Tensor.set_float2 map_y_ij x y 1.);
                Tensor.reshape map_y_ij ~shape:(1 :: Tensor.shape map_y_ij))
            in
            let map_channel_y = Tensor.concat channel_list ~dim:0 in
            Tensor.reshape map_channel_y ~shape:(1 :: Tensor.shape map_channel_y))
        in
        let map_batch_y = Tensor.concat batch_list ~dim:0 in
        Tensor.(map_batch_y * dx))
      else dx)
  in
  (y, dy) |> assert_right_shape "dropout4d"
