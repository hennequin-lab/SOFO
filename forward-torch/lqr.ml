...
let extract_list_opt list ~shape ~device =
  match list with
  | None -> Maths.const (Tensor.zeros shape ~device)
  | Some list -> List.last_exn list

let extract_list_opt_tensor list ~shape ~device =
  match list with
...
let lqr ~(state_params : state_params) ~(cost_params : cost_params) =
  ...
  let v_vec_final = extract_list_opt c_x_list ~shape:[ m; a_dim ] ~device in
  let backward t v_mat_next v_vec_next =
    let c_uu_curr = extract_list c_uu_list t in
    let c_xx_curr = extract_list c_xx_list t in
    let c_xu_curr = extract_list_opt c_xu_list ~shape:[ a_dim; b_dim ] ~device in
    let c_x_curr = extract_list_opt c_x_list ~shape:[ m; a_dim ] ~device in
    let c_u_curr = extract_list_opt c_u_list ~shape:[ m; b_dim ] ~device in
    let f_x_curr = extract_list f_x_list t in
    let f_u_curr = extract_list f_u_list t in
    let f_t_curr = extract_list f_t_list t in
    ...
    let q_u_curr =
      match f_t_curr with
      | None -> Maths.(c_u_curr + (v_vec_next *@ f_u_curr))
      | Some f_t_curr ->
        Maths.(c_u_curr + (v_vec_next *@ f_u_curr) + (f_t_curr *@ v_mat_next *@ f_u_curr))
    in
    let q_x_curr =
      match f_t_curr with
      | None -> Maths.(c_x_curr + (v_vec_next *@ f_x_curr))
      | Some f_t_curr ->
...
let lqr_tensor ~(state_params : state_params_tensor) ~(cost_params : cost_params_tensor) =
  ...
  (* step 1: backward pass to calculate K_t and k_t *)
  let v_mat_final = List.last_exn c_xx_list in
  let v_vec_final = extract_list_opt_tensor c_x_list ~shape:[ m; a_dim ] ~device in
  let backward t v_mat_next v_vec_next =
    let c_uu_curr = extract_list c_uu_list t in
    let c_xx_curr = extract_list c_xx_list t in
    let c_xu_curr = extract_list_opt_tensor c_xu_list ~shape:[ a_dim; b_dim ] ~device in
    let c_x_curr = extract_list_opt_tensor c_x_list ~shape:[ m; a_dim ] ~device in
    let c_u_curr = extract_list_opt_tensor c_u_list ~shape:[ m; b_dim ] ~device in
    let f_x_curr = extract_list f_x_list t in
    let f_u_curr = extract_list f_u_list t in
    let f_t_curr = extract_list f_t_list t in
    let q_uu_curr =
    ...
    let q_u_curr =
      match f_t_curr with
      | None -> Tensor.(c_u_curr + matmul v_vec_next f_u_curr)
      | Some f_t_curr ->
        Tensor.(
          c_u_curr
    ...
    let q_x_curr =
      match f_t_curr with
      | None -> Tensor.(c_x_curr + matmul v_vec_next f_x_curr)
      | Some f_t_curr ->
        Tensor.(
          c_x_curr
