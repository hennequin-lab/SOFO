...
module O = Optimizer.SOFO (MLP_mixer.P)
...
let rec loop ~t ~out ~state ~alpha_opt running_avg =
  ...
  let running_avg =
    ...
    then (
      ...
      in
      print [%message (e : float) (loss_avg : float) (alpha_opt : float)];
      Owl.Mat.(
        save_txt
          ~append:true
          ~out
          (of_array [| Float.of_int t; loss_avg; test_acc; train_acc; alpha_opt |] 1 5)));
    []
  in
  if t < max_iter
...
let _ =
  ...
  loop ~t:0 ~out ~state:(O.init MLP_mixer.init) ~alpha_opt:0.01 []
