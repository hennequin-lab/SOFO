...
(* let _ = Arr.save_npy ~out:(in_dir "lorenz_train") train_data *)
...
let _ =
  List.iter lr_rates ~f:(fun eta ->
    List.iter damping_list ~f:(fun gamma ->
      let config = config ~base_lr:eta ~gamma in
      let f_name =
        sprintf
          "rnn_%s_lr_%s_adaptive_lr_%s"
          meth
          (Float.to_string eta)
          (Bool.to_string config.adaptive_lr)
      in
      ...
      iter
        ...
        ~config
        ~t:0
        ~state:(O.init ~config RNN.(init ~d ~dh))
        ~time_elapsed:0.
        []))
...
(* let lr_rates = [ 0.0002 ]
   ...
   let _ =
   List.iter lr_rates ~f:(fun eta ->
   let config = config ~base_lr:eta ~gamma:None in
   let f_name = sprintf "rnn_%s_lr_%s" meth (Float.to_string eta) in
   Bos.Cmd.(v "rm" % "-f" % in_dir f_name) |> Bos.OS.Cmd.run |> ignore;
