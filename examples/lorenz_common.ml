...
(* Hernandez r2k *)
let r2k ~pred_true test_horizon =
  let pred_o = Mat.get_slice [ []; [ 0; 2 ] ] pred_true in
  let true_o = Mat.get_slice [ []; [ 3; 5 ] ] pred_true in
    ...
    Mat.init 1 30 (fun k ->
      let tmp =
        Array.init
          (test_horizon - k - 2)
          ~f:(fun t ->
            let true_o_t = Mat.get_slice [ [ t + k ] ] true_o in
            let pred_o_t = Mat.get_slice [ [ t + k ] ] pred_o in
            let num = Mat.(l2norm_sqr' (true_o_t - pred_o_t)) in
            let denom = Mat.(l2norm_sqr' (true_o_t - mean_o)) in
            num, denom)
      in
      Float.(
        1. - (Owl.Stats.sum (Array.map ~f:fst tmp) / Owl.Stats.sum (Array.map ~f:snd tmp))))
  in
  r2k
