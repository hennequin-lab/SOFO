...
module Make_LDS (X : module type of Default) = struct
  ...
  let a_tot_from_a a_lower_tri =
    let id_a_half = Mat.eye Int.(X.a / 2) in
    let zero_a_half = Mat.zeros Int.(X.a / 2) Int.(X.a / 2) in
    let a_upper = Mat.(zero_a_half @|| id_a_half) in
    let a_lower = Mat.(id_a_half @|| a_lower_tri) in
    Mat.(a_upper @= a_lower)

  (* generate parameters for lds *)
  let lds_params =
    (* if coupling layer dynamics *)
    let a_lower_tri = sample_a Int.(X.a / 2) in
    let a_tot = a_tot_from_a a_lower_tri in
    let b_tot =
      let b_upper =
        Mat.gaussian ~mu:0. ~sigma:Float.(1. / of_int X.a) Int.(X.a / 2) X.b
      in
      let b_lower = Mat.zeros Int.(X.a / 2) X.b in
      Mat.(b_upper @= b_lower)
    in
  ...
  let minibatch_as_data minibatch =
    ...
    let x_0 =
      let x_0_array =
        Array.init bs ~f:(fun i -> Mat.(reshape minibatch.(i).xs.(0) [| 1; -1 |]))
      in
      Mat.concatenate x_0_array ~axis:0
    in
    ...
    let x_u_list =
      List.init X.n_steps ~f:(fun t ->
        let x =
          let x_array =
            Array.init bs ~f:(fun i ->
              Mat.(reshape minibatch.(i).xs.(Int.(t + 1)) [| 1; -1 |]))
          in
          Mat.concatenate x_array ~axis:0
        in
        let u =
          let u_array =
            Array.init bs ~f:(fun i -> Mat.(reshape minibatch.(i).us.(t) [| 1; -1 |]))
          in
