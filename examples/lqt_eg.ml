...
(* -----------------------------------------
   ...
   ----------------------------------------- *)
module Lds_params_dim_tan = Lds_data.Default_Tan
...
let x_list, u_list = Lqt.lqt ~state_params ~x_u_desired ~cost_params
let _ = List.iter x_list ~f:(fun x -> Tensor.print (Maths.primal x))
